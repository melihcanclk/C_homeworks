...
int main(){
	...
	}else if(selection == 'N'){																	//Read Function
 	...
 	while(k<counter){	
		for(i=0;i<4;i++){
			...
			int lenght = strlen(scan_character);													//Finding Length Of The Word
				...
				if(scan_character[0] >= '0' && scan_character[0] <= '9'  ){							//Filtering For Detecting ID Numbers To Temporary Array
 				
 					strcpy(p[k].ID,scan_character);													//Copying Temporary Detected ID Number To Pointer
						
				}else if(scan_character[lenght - 4] == '.'|| scan_character[lenght - 3] == '.'){					//Filtering For Detecting E-Mail To Temporary Array
 			
 						strcpy(p[k].email,scan_character);											//Copying Temporary Detected E-Mail To Pointer
 							
 				}else if(scan_character[0] >= 'A' &&  scan_character[0] <= 'Z'  && scan_character[1] >= 'A' && scan_character[1] <= 'Z'  ){	//Filtering For Detecting Surname To Temporary Array
				
						strcpy(p[k].surname,scan_character);						//Copying Temporary Detected Surname To Pointer
						//printf("%s\n",p[k].surname);
								
				}else if(scan_character[0] >= 'A' &&  scan_character[0] <= 'Z'  && scan_character[1] >= 'a' && scan_character[1] <= 'z' ){	//Filtering For Detecting Name To Temporary Array
 					x=0;
 					while(scan_character[x]!='\0'){								//Counting How Many Words Are There In The Scanned Word
 							x++;
 					}					
 					strcpy(p[k].name,scan_character);							//Copying Temporary Detected E-Mail To Pointer
 						...
 						if(scan_character[0] >= 'A' &&  scan_character[0] <= 'Z'  && scan_character[1] >= 'a' && scan_character[1] <= 'z' ) {		//Again Filtering For Detecting Name To Temporary Array
								
								p[k].name[x]=' ';
								p[k].name[x+1]='\0';									//Ascending Space End Of The First Number
 								strcat(p[k].name,scan_character);						//Adding Two Word Together
 								
 							}else if(scan_character[0] >= '0' && scan_character[0] <= '9'  ){		//Filtering For Detecting ID Numbers To Temporary Array
 		...
 		for (i = 0; i < counter+1; i++){	
 			for (j = i+1; j < counter; j++){
				...
				b=strlen((p+j)->ID);
 				...
 				if(a>b){																				//If Lenght Of First ID Of Array is Bigger Than Second ID Of Array,Reverse Them.  
 				...
 				}if(a==b){																					//If They Are Equal, Look At The Numbers Of ID's 
 				char *ptr;
 				int count=0;
 				
 				for(ptr=(p+j)->ID;ptr< (char*)(p+j)->ID+1;ptr++){											//ID's Character Numbers
        			count++;
 				
 				}
 				for(x=0; x<count;x++){
 					if((p+j)->ID[x] <=(p+i)->ID[x]){														//If Lenght Of First Number Of ID is Bigger Than Second Number Of ID,Reverse Them.  
...
void writeDataToFile(record *p,int counter){
	...
	while(c<1){
		...
		switch(input_char){
			...
			for(i=0;i<counter;i++){															//Line Increaser Loop
				...
				if(lenght_surname <4)
				{
					...
					fprintf(fileopen_write,"%s				",p[i].surname);
				}else if(lenght_surname <12){												//Printinf Regularly. I Could Use '->' Instead Of '.'
					fprintf(fileopen_write,"%s			",p[i].surname);
				}else{
					fprintf(fileopen_write,"%s		",p[i].surname);
				}
				if(lenght_email <4)
				{
					...
					fprintf(fileopen_write,"%s				",p[i].email);
				}else if(lenght_email <12){													//Printinf Regularly. I Could Use '->' Instead Of '.'
					fprintf(fileopen_write,"%s			",p[i].email);
				}else{
					fprintf(fileopen_write,"%s		",p[i].email);
				}
		...
		case 'M':
			...
			for(i=0;i<counter;i++){															//Line Increaser Loop
				...
				if(lenght_email <8)
				{
					...
					fprintf(fileopen_write,"%s						",p[i].email);
				}else if(lenght_email <24){													//Printinf Regularly. I Could Use '->' Instead Of '.'
					fprintf(fileopen_write,"%s				",p[i].email);
				}else{
					fprintf(fileopen_write,"%s			",p[i].email);
				}
